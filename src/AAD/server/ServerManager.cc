...
/* ============================================================================
 * RosBridge Process Management
 * ========================================================================== */

 void ServerManager::startRosBridge(){
    if (_rosbridgeProcess) {
        qWarning() << "RosBridge already running";
        return;
    }

    _rosbridgeProcess = new QProcess(this);
    _rosbridgeProcess->setProcessChannelMode(QProcess::MergedChannels);

    connect(_rosbridgeProcess,
            &QProcess::readyReadStandardOutput,
            this, [this]() {

        const QString text =
            QString::fromUtf8(_rosbridgeProcess->readAllStandardOutput());
    ...
    connect(_rosbridgeProcess,
        ...
        appendLog(
            QString("RosBridge exited: %1 (%2)").arg(code).arg(status)
        );

        _rosbridgeProcess->deleteLater();
        _rosbridgeProcess = nullptr;
        emit rosbridgeRunningChanged();
    });

    const QString wrapper = QString(QGC_SOURCE_DIR) + "/src/AAD/RosBridge/run_rosbridge.sh";

    if (QFile::exists(wrapper)) {
        // startDetached returns (pid) on success
        _rosbridgeProcess->start(wrapper);

        if (!_rosbridgeProcess->waitForStarted(1500)) {
            appendLog(QString("Failed to start [RosBridge]: %1").arg(wrapper));
            return;
        }

        appendLog("RosBridge started");
    } else {
        qWarning() << "RosBridge wrapper not found at" << wrapper;
    }

    emit rosbridgeRunningChanged();
 }

 void ServerManager::stopRosBridge(){ if (_rosbridgeProcess) _rosbridgeProcess->terminate(); }

bool ServerManager::rosbridgeRunning() const
{
    return _rosbridgeProcess &&
           _rosbridgeProcess->state() != QProcess::NotRunning;
}
