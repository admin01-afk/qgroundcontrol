...
bool MissionCommandUIInfo::loadJsonInfo(const QJsonObject& jsonObject, bool requireFullObject, QString& errorString)
{
    ...
    for (int i=1; i<=7; i++) {
        ...
        if (jsonObject.contains(paramKey)) {
            ...
            if (!paramObject.contains(_advancedJsonKey)) {
                internalError = QString("param object missing advanced key: %1").arg(paramKey);
                errorString = _loadErrorString(internalError);
                return false;
            }
            ...
            paramInfo->_advanced =      paramObject.value(_advancedJsonKey).toBool();
